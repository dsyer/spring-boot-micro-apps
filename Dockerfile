...
COPY src ./src
COPY *.json ./
RUN --mount=type=cache,target=/root/.m2 ./mvnw dependency:get -Dartifact=org.springframework.boot.experimental:spring-boot-thin-launcher:1.0.21.RELEASE:jar:exec -Dtransitive=false
RUN --mount=type=cache,target=/root/.m2 ./mvnw install -DskipTests

FROM dsyer/graalvm-native-image:1.0.0-rc14 as native
WORKDIR /workspace/app
ARG SAMPLE=com.example.func.BuncApplication
ARG THINJAR=/root/.m2/repository/org/springframework/boot/experimental/spring-boot-thin-launcher/1.0.21.RELEASE/spring-boot-thin-launcher-1.0.21.RELEASE-exec.jar
